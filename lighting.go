...
type LightImages struct {
	...
	rectangleLight1      image.Rectangle
}

// this returns a LightImages struct to init the one in the Game struct
func initLightImages() LightImages {
	return LightImages{
		playerLight:          image.Rect(0, 0, 84, 84),
		bulletLight:          image.Rect(0, 84, 44, 129),
		bulletExplosionLight: image.Rect(0, 129, 34, 163),
		rectangleLight1:      image.Rect(0, 166, 20, 366),
	}
}
...
// Same as above except includes a position (and will be used with a different update function/no need for one)
func createLightStatic(lightRect image.Rectangle, id int, rotation float64, position Vec2f) Light {
	return Light{
		subImage: lightRect,
		image:    ilightingSpritesheet,
		id:       id,
